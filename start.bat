...
echo [INFO] Auto-detecting GPU...

REM Check for NVIDIA GPU via Python (more reliable than nvidia-smi in batch)
...
if %HAS_GPU% == 1 (
    ...
    REM Check if PyTorch is already installed (any version)
    ...
    if errorlevel 1 (
        ...
        if errorlevel 1 (
            echo [INFO] Converting from CUDA to CPU version...
            pip uninstall torch torchvision torchaudio -y
            pip install torch torchvision
            echo [OK] CPU version installed!
        ) else (
...
pip install numpy opencv-python Pillow trimesh scipy pystray pytz networkx lxml colormath gradio
echo.
