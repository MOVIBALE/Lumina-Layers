"""
Lumina Studio - PyTorch CUDA Accelerated Image Processing Core (Beta)
...
# Try to import torch with error handling for non-CUDA systems
try:
    import torch
...
class LuminaImageProcessorCUDA:
        ...
        if TORCH_AVAILABLE:
            try:
                ...
                if not self.use_cuda and hasattr(torch, 'version') and hasattr(torch.version, 'hip'):
                    try:
                        # ROCm uses the same torch.cuda API but with ROCm backend
                        if torch.cuda.is_available():
                            self.use_rocm = True
                            self.use_cuda = True  # ROCm uses CUDA API compatibility
                    except:
                        pass
    ...
    def _load_lut(self, lut_path):
        ...
        if "6-Color" in self.color_mode or total_colors == 1296:
            ...
            print(f"[OK] LUT loaded: {len(self.lut_rgb)} colors (6-Color mode)")
        ...
        # Branch 2: 4-Color Standard (1024)
            ...
            print(f"[OK] LUT loaded: {len(self.lut_rgb)} colors (filtered {dropped} outliers)")
        ...
        if TORCH_AVAILABLE and self.use_cuda:
            try:
                ...
                print(f"[CUDA] LUT loaded on GPU")
            except Exception as e:
    ...
    def _process_pixel_mode_cuda(self, rgb_arr, target_h, target_w):
        ...
        if TORCH_AVAILABLE and self.use_cuda and len(flat_rgb) > 10000:
            try:
